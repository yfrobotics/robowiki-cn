...
典型的 SLAM 系统由前端、后端与回环检测三部分构成，各部分的职责与数据流如下图所示：

![SLAM 系统框架](assets/slam-framework.svg)
...
位姿图（或引入路标节点后的因子图）的结构如下。里程计边由相邻帧的位姿估计产生，误差会沿链条累积；回环边把相隔很远的两个位姿直接约束在一起，优化时将累积误差分摊到整条轨迹上，这是 SLAM 区别于纯里程计的关键。

![位姿图与因子图结构](assets/slam-pose-graph.svg)
