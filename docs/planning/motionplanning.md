...
工作空间中的障碍物映射到构型空间中形成 C-space 障碍物 \(\mathcal{C}_{obs}\)。对于平面上的圆盘机器人，这一映射就是把障碍物按机器人半径向外膨胀，同时把机器人本身收缩为一个点：

![工作空间障碍物到构型空间障碍物的映射](assets/cspace-mapping.svg)

这样处理之后，碰撞检测就与机器人的形状无关，退化为"某个点是否落在 \(\mathcal{C}_{obs}\) 内"。自由构型空间定义为：
