...
![ICP 点云配准的迭代收敛过程](assets/icp-iteration.svg)

上图是一次真实运行的记录：源点云初始偏离目标 38 度、平移 (0.55, -0.35)，前几次迭代误差下降很快，随后逐渐收敛，30 次迭代后精确恢复出真值。注意误差下降是单调的——这是 ICP 的一个基本性质，但它只保证收敛到局部最优。
