...
动作块能够建模连续动作之间的关系，并降低推理调用频率，一次预测可以覆盖 \(H\) 个控制周期。但执行期间缺少新观测反馈，也可能加剧状态偏离；它并不自动消除[复合误差](embodied-ai-data.md)。实践中可以只执行动作块的前几步，再利用新观测重新规划，具体见[动作块时序与闭环评测](vla-training-evaluation.md)。
...
完整的数据检查、训练记录、动作接口与评测流程见[VLA 微调与评测](vla-training-evaluation.md)。公开演示的格式转换与混合策略见[开放机器人数据集实践](../database/open-robot-datasets.md)。
