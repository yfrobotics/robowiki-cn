...
下图给出同一任务集在两种调度策略下的实际时序。该任务集总利用率 U = 0.971，超过了 RM 的充分条件上界 0.828 但未超过 EDF 的充要条件上界 1.0——RM 在 t = 7 就错过了截止时限，EDF 则在整个超周期内都能满足：

![RM 与 EDF 调度时序对比](assets/rm-vs-edf.svg)
...
![优先级反转与优先级继承的时序对比](assets/priority-inversion.svg)

上图左半部分显示了问题的本质：H 的阻塞时长并不取决于 L 的临界区长度，而取决于所有中优先级任务的执行时间之和——只要 M 类任务足够多，阻塞就可以任意长。引入优先级继承后（下半部分），L 在持锁期间临时提升到 H 的优先级，M 无法抢占，阻塞时间被约束为临界区的剩余长度。
